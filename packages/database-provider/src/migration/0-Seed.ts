...
export class Seed1617737108784 implements MigrationInterface {
  ...
  private async seed_ranking (queryRunner: QueryRunner): Promise<void> {
    ...
    for (let i = 0; i < rank.length; i++) {
      await queryRunner.query(`insert into ranking values(default,default,default,default,'${i < 4 ? RankingType.SEN : RankingType.VET}')`)
    }
    const ids: string[] = (await queryRunner.query('select id from ranking;')).map((x: any) => x.id)

    for (const [i, id] of ids.entries()) {
      await queryRunner.query(`insert into ranking_players_player values('${id}','${rank[i]}')`)
    }
  }
}
